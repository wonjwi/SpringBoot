...
public class MemberController {
	...
	public String login(MemberDTO memberDTO, HttpServletRequest req, 
		...
		if(login == null) {
			...
			//addAttribute를 사용하면, 변수명과 값이 주소에 노출된다.
			//rttr.addAttribute("msg", false);
			
			//addFlashAttribute를 사용하여 변수명과 주소값을 노출시키지 않고 사용한다.
	...
	public String logout(HttpSession session) throws Exception {
		...
		//로그아웃 버튼을 눌렀을 경우, 세션을 없애고 로그인 화면으로 돌아가도록 한다.
		session.invalidate();
		return "redirect:/member/login";
	}
	
	// 수정할 화면을 보여달라고 하는 회원정보 수정 요청이 들어왔다.
	// 회원정보 수정 GET
	...
	public String memberDelete(MemberDTO memberDTO, HttpSession session,
		...
		//세션에 들어있는 memeberDTO 정보를 가져와서 member 변수에 저장한다.
		...
		//msg에 false를 담아서 회원탈퇴 화면으로 이동한다
		if( !(sessionPasswd.equals(memberDTOPasswd)) ) {
			rttr.addFlashAttribute("msg", false);
			return "redirect:/member/memberDelete";
		}
		//세션의 비밀번호와 입력한 비밀번호가 맞으면
		//회원탈퇴 작업 => Service에게 회원탈퇴 작업을 시킨다.
